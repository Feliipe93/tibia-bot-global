...
# -*- coding: utf-8 -*-
"""
Script de diagnóstico para el sistema de targeting (versión offline).
Verifica la detección de criaturas en la battle list usando un frame guardado.
"""
...
def diagnose_targeting_offline(frame_path="debug/targeting_diagnosis_frame.png"):
    """Diagnostica el sistema de targeting usando un frame guardado."""
    
    print("=== DIAGNÓSTICO DE TARGETING (OFFLINE) ===\n")
    
    # 1. Cargar frame
    print(f"1. Cargando frame desde {frame_path}...")
    if not os.path.exists(frame_path):
        print(f"X No existe el archivo: {frame_path}")
        print("   Primero ejecuta el bot para que capture frames o captura uno manualmente")
        return
    
    frame = cv2.imread(frame_path)
    if frame is None:
        print(f"X No se pudo cargar el frame: {frame_path}")
        return
    ...
    print("\n8. RECOMENDACIONES:")
    if len(creatures) == 0:
        print("   - No se detectaron criaturas. Posibles causas:")
        print("     * Los templates no coinciden con los nombres en la battle list")
        print("     * La battle region está mal calibrada")
        print("     * El umbral de detección (name_precision) es muy alto")
        print("     * No hay criaturas en la battle list actualmente")
        
        print("\n   - Soluciones sugeridas:")
        print("     * Verifica que los templates en images/Targets/Names/ coincidan con los nombres")
        print("     * Captura nuevos templates si los nombres tienen diferente fuente/color")
        print("     * Reduce el umbral de name_precision en battle_list_reader.py")
        print("     * Asegúrate de que haya monstruos visibles en la battle list")
    else:
        print("   - Criaturas detectadas correctamente")
        print("   - Si el targeting no ataca, revisa el log del targeting engine")
