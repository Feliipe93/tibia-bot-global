...
# -*- coding: utf-8 -*-
"""
rotworm_visual_recognition.py - Demostración de cómo el bot reconoce visualmente a Rotworm
"""
...
def main():
    ...
    print(f"   │  Look Type: {rotworm_info.look_type:<15} │")
    print(f"   │  Outfit Type: {rotworm_info.outfit_type:<12} │")
    print(f"   │  Head: {rotworm_info.look_head:<18} │")
    print(f"   │  Body: {rotworm_info.look_body:<17} │")
    print(f"   │  Legs: {rotworm_info.look_legs:<17} │")
    print(f"   │  Feet: {rotworm_info.look_feet:<17} │")
    print(f"   │  Addons: {rotworm_info.look_addons:<15} │")
    print(f"   └─────────────────────────────────────────┘")
    
    # 3. Crear template visual
    print(f"\n3. TEMPLATE VISUAL PARA DETECCIÓN:")
    print(f"   El bot usa estos datos para reconocer Rotworm:")
    print(f"   - Look Type único: {rotworm_info.look_type}")
    print(f"   - Color de cuerpo: {rotworm_info.look_body}")
    print(f"   - Color de piernas: {rotworm_info.look_legs}")
    print(f"   - Tipo de outfit: {rotworm_info.outfit_type}")
    ...
    if corpse_info:
        ...
        print(f"   │  Nombre: {corpse_info.name:<25} │")
        print(f"   │  Item ID: {corpse_info.item_id:<22} │")
        print(f"   │  Descripción: {corpse_info.description:<18} │")
        print(f"   │  Tamaño: {corpse_info.size:<22} │")
        print(f"   │  Duración: {corpse_info.decay_time:<18} │")
        print(f"   └─────────────────────────────────────────┘")
    
    # 5. Proceso de reconocimiento con OBS
    ...
    # 6. Ejemplo de código de detección
    print(f"\n6. EJEMPLO DE CÓDIGO DE DETECCIÓN:")
    print(f"   ```python")
    ...
    # 7. Características visuales específicas
    print(f"\n7. CARACTERÍSTICAS VISUALES ESPECÍFICAS:")
    ...
    # 8. Sistema de reconocimiento mejorado
    print(f"\n8. SISTEMA DE RECONOCIMIENTO MEJORADO:")
